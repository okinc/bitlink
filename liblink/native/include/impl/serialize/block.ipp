 ...
 */
#ifndef BITLINK_IMPL_SERIALIZE_BLOCK_IPP
#define BITLINK_IMPL_SERIALIZE_BLOCK_IPP
...
Iterator satoshi_save(const transaction_type& packet, Iterator result)
{
    ...
    BOOST_FOREACH (const transaction_input_type& input, packet.inputs)
    {
    ...
    BOOST_FOREACH (const transaction_output_type& output, packet.outputs)
    {
...
{
	deserializer<Iterator> deserial = make_deserializer(first, last);
...
Iterator satoshi_save(const block_type& packet, Iterator result)
{
    ...
    serializer<Iterator> serial = make_serializer(result + 80);
    serial.write_variable_uint(packet.transactions.size());
    Iterator write_iter = serial.iterator();
    BOOST_FOREACH (const transaction_type& tx, packet.transactions)
    ...
    block_type& packet)
{
    ...
    deserializer<Iterator> deserial = make_deserializer(first + 80, last);
...
#endif /* BITLINK_IMPL_SERIALIZE_BLOCK_IPP */
