 ...
 */
#ifndef BITLINK_IMPL_SATOSHI_SERIALIZE_IPP
#define BITLINK_IMPL_SATOSHI_SERIALIZE_IPP
...
{
    serializer<Iterator> serial = make_serializer(result);
...
void satoshi_load(const Iterator first, const Iterator last,
    header_type& head)
{
    serializer<Iterator> deserial = make_deserializer(first, last);
...
Iterator satoshi_save(const address_type& packet, Iterator result)
{
    ...
    BOOST_FOREACH (const network_address_type& net_address, packet.addresses)
    {
...
uint32_t inventory_type_to_number(inventory_type_id::inventory_type_id inv_type);
inventory_type_id::inventory_type_id inventory_type_from_number(uint32_t raw_type);
...
Iterator save_inventory_impl(const Message& packet, Iterator result)
{
    ...
    BOOST_FOREACH (const inventory_vector_type inv, packet.inventories)
    {
...
Iterator satoshi_save(const get_blocks_type& packet, Iterator result)
{
    ...
    BOOST_FOREACH (hash_digest start_hash, packet.start_hashes)
...
#endif /* BITLINK_IMPL_SATOSHI_SERIALIZE_IPP */
