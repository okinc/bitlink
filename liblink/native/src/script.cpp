...
static const data_chunk stack_true_value =  boost::assign::list_of(1);
...
{
    for (data_chunk::const_iterator it = values.begin(); it != values.end(); ++it)
    {
        if (*it != 0)
        {
...
bool is_push(const opcode::opcode code)
{
...
{
	int count = 0;
	BOOST_FOREACH (const operation& op, operations)
	{
		if(!is_push(op.code))
		{
			count++;
    ...
    uint32_t input_index, bool bip16_enabled)
{
    ...
    if (bip16_enabled && type() == script_hash)
    {
...
bool opcode_is_disabled(opcode::opcode code)
{
    switch (code)
    {
        case opcode::cat:
...
bool script_type::run(const transaction_type& parent_tx, uint32_t input_index)
{
    ...
    for (operation_stack::iterator it = operations_.begin(); it != operations_.end(); ++it)
...
static bool is_condition_opcode (opcode::opcode code)
{
...
bool script_type::op_x(opcode::opcode code)
{
    uint8_t value_diff =
...
bool script_type::op_2dup()
{
    if (stack_.size() < 2)
        return false;
    data_chunk dup_first = *(stack_.end() - 2),
...
bool script_type::op_3dup()
{
    if (stack_.size() < 3)
        return false;
    data_chunk dup_first = *(stack_.end() - 3),
...
bool pick_roll_impl(DataStack& stack, bool is_roll)
{
    ...
    typedef typename DataStack::iterator _DataStack_iterator;
    _DataStack_iterator slice_iter = stack.end() - n - 1;
...
bool script_type::op_ripemd160()
{
    ...
    RIPEMD160(data.begin(), data.size(), hash.begin());
...
bool script_type::op_sha1()
{
    ...
    SHA1(data.begin(), data.size(), hash.begin());
    stack_.push_back(hash);
    return true;
}
...
bool script_type::op_sha256()
{
    ...
    SHA256(data.begin(), data.size(), hash.begin());
    stack_.push_back(hash);
    return true;
}
    ...
    const script_type& script_code, uint32_t hash_type)
{
    ...
    {
//TODO        log_fatal(LOG_SCRIPT) << "script_type::op_checksig() : input_index "
// TODO       << input_index << " is out of range.";
        return one_hash();
    }
    ...
    BOOST_FOREACH (transaction_input_type& input, parent_tx.inputs)
    ...
    else if ((hash_type & 0x1f) == sighash::single)
    {
        ...
        {
//TODO            log_error(LOG_SCRIPT)
// TODO                << "sighash::single the output_index is out of range";
            return one_hash();
        }
        outputs.resize(output_index + 1);
        // Loop through outputs except the last one
        for (transaction_output_list::iterator it = outputs.begin(); it != outputs.end() - 1; ++it)
        {
    ...
    const transaction_type& parent_tx, uint32_t input_index)
{
    ...
    for (operation_stack::const_iterator it = codehash_begin_; it != operations_.end(); ++it)
    {
        const operation op = *it;
...
static bool is_signature (const data_stack &signatures, const data_chunk& data)
{
    ...
    const transaction_type& parent_tx, uint32_t input_index)
{
    ...
    for (operation_stack::const_iterator it = codehash_begin_; it != operations_.end(); ++it)
    {
        ...
        if (op.code == opcode::codeseparator)
            continue;
        if (is_signature(signatures, op.data))
            continue;
        script_code.push_operation(op);
    }
    ...
    data_stack::iterator pubkey_current = pubkeys.begin();
    BOOST_FOREACH (const data_chunk& signature, signatures)
    {
        for (data_stack::iterator pubkey_iter = pubkey_current; ;)
        {
        ...
        const transaction_type& parent_tx, uint32_t input_index)
{
    switch (op.code)
    {
        ...
        default:
//TODO            log_fatal(LOG_SCRIPT) << "Unimplemented operation <none "
// TODO                << static_cast<int>(op.code) << ">";
            return false;
    }
    return false;
}
...
payment_type script_type::type() const
{
    ...
    if (is_pubkey_hash_sig_type(operations_))
        return pubkey_hash_sig;
    if (is_script_code_sig_type(operations_))
        return script_code_sig;
    return non_standard;
}

std::string opcode_to_string(opcode::opcode code)
{
    switch (code)
    {
        case opcode::zero:
...
opcode::opcode string_to_opcode(const std::string& code_repr)
{
    if (code_repr == "zero")
        return opcode::zero;
...
std::string pretty(const script_type& script)
{
    ...
    std::ostringstream ss;
    for (operation_stack::const_iterator it = opers.begin(); it != opers.end(); ++it)
    {
        if (it != opers.begin())
            ss << " ";
        const operation& op = *it;
...
bool must_read_data(opcode::opcode code)
{
...
size_t number_bytes_to_read(
    opcode::opcode code, uint8_t raw_byte, Deserializer& deserial)
{
    switch (code)
    {
        case opcode::special:
...
{
    script_type script_object;
    deserializer<data_chunk::const_iterator> deserial = make_deserializer(raw_script.begin(), raw_script.end());
    while (deserial.iterator() != raw_script.end())
    {
        uint8_t raw_byte = deserial.read_byte();
        operation op;
        op.code = static_cast<opcode::opcode>(raw_byte);
...
inline data_chunk operation_metadata(const opcode::opcode code, size_t data_size)
{
    switch (code)
    {
        case opcode::pushdata1:
...
data_chunk save_script(const script_type& script)
{
    ...
    BOOST_FOREACH (const operation& op, script.operations())
    {
