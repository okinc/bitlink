...
{
    return hash_transaction_impl(tx, NULL);
}
hash_digest hash_transaction(const transaction_type& tx,
    uint32_t hash_type_code)
{
...
hash_digest build_merkle_tree(hash_list& merkle)
{
    ...
    while (merkle.size() > 1)
    {
        ...
        for (hash_list::const_iterator it = merkle.begin(); it != merkle.end(); it += 2)
        {
            data_chunk concat_data(hash_digest_size * 2);
            serializer<data_chunk::iterator> concat = make_serializer(concat_data.begin());
...
{
    hash_list tx_hashes;
    BOOST_FOREACH (transaction_type tx, transactions)
...
std::string pretty(const transaction_type& tx)
{
    ...
    ss << "Transaction:\n"
        ...
        << "Inputs:\n";
    BOOST_FOREACH (transaction_input_type input, tx.inputs)
        ss << pretty(input);
    ss << "Outputs:\n";
    BOOST_FOREACH (transaction_output_type output, tx.outputs)
        ss << pretty(output);
    ss << "\n";
    return ss.str();
}
...
{
    uint64_t total = 0;
    BOOST_FOREACH (const transaction_output_type& output, tx.outputs)
        total += output.value;
    return total;
}
    ...
    size_t block_height, uint32_t block_time)
{
    ...
    if (tx.locktime < max_locktime)
        return true;
    BOOST_FOREACH (const transaction_input_type& tx_input, tx.inputs)
