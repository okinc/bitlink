...
bool elliptic_curve_key::set_public_key(const data_chunk& pubkey)
{
    if (!initialize())
        return false;
    const unsigned char* pubkey_bytes = pubkey.begin();
...
data_chunk elliptic_curve_key::public_key() const
{
    ...
    uint8_t* pubkey_begin = pubkey.begin();
...
{
    BITCOIN_ASSERT(key_ != NULL);
    // SSL likes a reversed hash
    std::reverse(hash.begin(), hash.end());
    // -1 = error, 0 = bad sig, 1 = good
    if (ECDSA_verify(0, hash.begin(), hash.size(),
            signature.begin(), signature.size(), key_) == 1)
        return true;
    return false;
}
...
bool elliptic_curve_key::set_private_key(const private_data& privkey)
{
    if (!initialize())
        return false;
    const uint8_t* privkey_begin = privkey.begin();
...
private_data elliptic_curve_key::private_key() const
{
    ...
    uint8_t* privkey_begin = privkey.begin();
...
data_chunk elliptic_curve_key::sign(hash_digest hash) const
{
    ...
    if (!ECDSA_sign(0, hash.data(), hash.size(),
            signature.begin(), &signature_length, key_))
        return data_chunk();
